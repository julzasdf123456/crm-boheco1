    ...
    public function dashboardReadingMonitor(Request $request) {
        ...
        if ($day == 'All') {
            $data = DB::table('Billing_ServiceAccounts')
                ...
                ->select('users.name', 'users.id',
                    DB::raw("(SELECT COUNT(r.id) FROM Billing_Readings r LEFT JOIN Billing_ServiceAccounts sa ON sa.id=r.AccountNumber WHERE sa.MeterReader=CAST(users.id AS varchar) AND r.ServicePeriod='" . $period . "' AND r.AccountNumber NOT IN (SELECT AccountNumber FROM Billing_Bills WHERE AccountNumber=r.AccountNumber AND ServicePeriod='" . $period . "')) AS TotalUnbilledBasedFromReadings"),
                    DB::raw("(SELECT COUNT(id) FROM Billing_ServiceAccounts WHERE MeterReader=CAST(users.id AS varchar) AND id NOT IN (SELECT AccountNumber FROM Billing_Bills WHERE AccountNumber=Billing_ServiceAccounts.id AND ServicePeriod='". $period ."')) AS AllUnbilled"),
                    ...
                    DB::raw("(SELECT SUM(TRY_CONVERT(DECIMAL(10,2), KwhUsed)) FROM Billing_Bills b LEFT JOIN Billing_ServiceAccounts sa ON sa.id=b.AccountNumber WHERE b.KwhUsed IS NOT NULL AND ISNUMERIC(b.KwhUsed)=1 AND sa.MeterReader=CAST(users.id AS varchar) AND b.ServicePeriod='" . $period . "') AS TotalKwh")
                )
                ->groupBy('users.name', 'users.id')
                ->orderBy('users.name')
                ->get();
            ...
            $data = DB::table('Billing_ServiceAccounts')
                ...
                ->select('users.name', 'users.id',
                    ...
                    DB::raw("(SELECT COUNT(r.id) FROM Billing_Readings r LEFT JOIN Billing_ServiceAccounts sa ON sa.id=r.AccountNumber WHERE sa.MeterReader=CAST(users.id AS varchar) AND sa.GroupCode='" . $day . "' AND r.ServicePeriod='" . $period . "' AND r.AccountNumber NOT IN (SELECT AccountNumber FROM Billing_Bills WHERE AccountNumber=r.AccountNumber AND ServicePeriod='" . $period . "')) AS TotalUnbilledBasedFromReadings"),
                    DB::raw("(SELECT COUNT(id) FROM Billing_ServiceAccounts WHERE MeterReader=CAST(users.id AS varchar) AND GroupCode='" . $day . "' AND id NOT IN (SELECT AccountNumber FROM Billing_Bills WHERE AccountNumber=Billing_ServiceAccounts.id AND ServicePeriod='". $period ."')) AS AllUnbilled"),
                    ...
                    DB::raw("(SELECT SUM(TRY_CONVERT(DECIMAL(10,2), KwhUsed)) FROM Billing_Bills b LEFT JOIN Billing_ServiceAccounts sa ON sa.id=b.AccountNumber WHERE b.KwhUsed IS NOT NULL AND ISNUMERIC(b.KwhUsed)=1 AND sa.GroupCode='" . $day . "' AND sa.MeterReader=CAST(users.id AS varchar) AND b.ServicePeriod='" . $period . "') AS TotalKwh")
                )
                ->groupBy('users.name', 'users.id')
                ->orderBy('users.name')
                ->get();
